...
public static class ComposerProjectGenerator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var projects = BuildProjectPlan(
            projectPrefix,
            resolvedIds.Contains(FoundationCapabilityIds.WebApi),
            resolvedIds.Contains(FoundationCapabilityIds.Blazor));
        ...
        var files = BuildGeneratedFiles(
            ...
            bindings,
            resolvedIds);
        files[GeneratedMarkerFile] = ComposerGeneratedOwnership.BuildMarker(
    ...
    {
        if (foundationRoot is not null && IsSameOrAncestor(outputDirectory, foundationRoot))
        {
            throw new ComposerGenerationException(
    ...
    private static void PrepareDestination(string outputDirectory, bool force)
    {
        ...
        {
            throw new ComposerGenerationException(
                "The generation destination is not empty. Re-run with --force only for an unchanged directory previously generated by FoundationKit Composer.");
        }

        ComposerGeneratedOwnership.ValidateUnchangedGeneratedDirectory(outputDirectory);

        var expectedFiles = ReadGeneratedMarker(outputDirectory);
        var actualFiles = Directory
            .EnumerateFiles(outputDirectory, "*", SearchOption.AllDirectories)
            .Select(path => NormalizeProjectPath(Path.GetRelativePath(outputDirectory, path)))
    ...
    private static string[] ReadGeneratedMarker(string outputDirectory)
    {
        var markerPath = Path.Combine(outputDirectory, GeneratedMarkerFile);
        ...
        try
        {
            ...
            if (!root.TryGetProperty("schemaVersion", out var schemaVersion) || schemaVersion.GetInt32() != 1 ||
                ...
                !root.TryGetProperty("generatedFiles", out var generatedFiles) ||
            ...
            if (files.Length == 0 ||
                !files.Contains(GeneratedMarkerFile, StringComparer.Ordinal) ||
                files.Any(IsUnsafeRelativePath))
            {
        ...
        GeneratedProject[] projects)
    {
        ...
        return PackageBindings
            .Where(binding => resolvedIds.Contains(binding.CapabilityId) && availableLayers.Contains(binding.Layer))
        ...
        GeneratedProject[] projects)
    {
        ...
        return layers.Select(layer => projects.Single(candidate => candidate.Layer == layer)).ToArray();
    }
    ...
    private static string BuildSolution(GeneratedProject[] projects)
    {
        ...
        foreach (var project in projects)
        {
            ...
            builder.Append("Project(\"").Append(CSharpProjectTypeGuid).Append("\") = \"")
                .Append(project.Name).Append("\", \"").Append(project.ProjectPath.Replace('/', '\\'))
        ...
        string referenceMode)
    {
        ...
        foreach (var entry in analysis.Entries)
        {
            ...
            builder.Append("| `").Append(entry.Capability.Id).Append("` | v")
                ...
                .Append(string.Join(", ", entry.Reasons.Select(EscapeMarkdownCell))).Append(" | ")
