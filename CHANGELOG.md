...
- Bounded local generation to a configured host workspace: Docker exposes FoundationKit `src` read-only and only `generated/` writable, and the browser never supplies a free filesystem path.
